...
-- MAGIC %sql
-- MAGIC CREATE SCHEMA IF NOT EXISTS main.lineage;
-- MAGIC CREATE TABLE IF NOT EXISTS main.lineage.menu (recipe_id INT, app string, main string, desert string);
-- MAGIC DELETE from main.lineage.menu ;
-- MAGIC 
-- MAGIC INSERT OVERWRITE main.lineage.menu
...
-- MAGIC %sql
-- MAGIC CREATE TABLE IF NOT EXISTS main.lineage.dinner 
...
-- MAGIC df = spark.range(3).withColumn("price", F.round(10*F.rand(seed=42),2)).withColumnRenamed("id", "recipe_id")
-- MAGIC 
-- MAGIC df.write.mode("overwrite").saveAsTable("main.lineage.price")
-- MAGIC 
-- MAGIC dinner = spark.read.table("main.lineage.dinner")
-- MAGIC price = spark.read.table("main.lineage.price")
-- MAGIC 
-- MAGIC dinner_price = dinner.join(price, on="recipe_id")
-- MAGIC dinner_price.write.mode("overwrite").saveAsTable("main.lineage.dinner_price")
...
-- MAGIC %sql
-- MAGIC GRANT SELECT ON TABLE main.lineage.dinner TO `account users`;
-- MAGIC GRANT SELECT ON TABLE main.lineage.dinner_price TO `account users`;
-- MAGIC GRANT SELECT ON TABLE main.lineage.menu TO `account users`;
-- MAGIC GRANT SELECT ON TABLE main.lineage.price TO `account users`;
